...
def concatenate_md_files(input_dir, output_file):
   with open(output_file, 'w') as outfile:
       intro_file = os.path.join('docx', 'intro.md')
       if os.path.exists(intro_file):
           with open(intro_file, 'r') as infile:
               content = replace_footnotes(infile.read())
               outfile.write(content + '\n\n')

       index_file = os.path.join(input_dir, 'index.md')
...
def copy_media_folder(input_dir, output_dir):
   media_src = os.path.join(input_dir, 'media')
...
if __name__ == "__main__":
   ...
   parser.add_argument('--input-dir', default='/wiki/docs', help='Input directory')
