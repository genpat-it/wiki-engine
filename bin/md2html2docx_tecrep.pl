...
example:
	cd PROJECT_DIR
	perl bin/md2html2docx_ref.pl   examples/example.md   template/template_tecrep.docx
};
#--------------------------------------------------
#  INPUT
#--------------------------------------------------
...
if($template eq '' ){
	if(-e 'template/template_tecrep.docx'){
		$template='template/template_tecrep.docx'
	}else{
		print $usage; 
		exit;		
	}
...
sub change_md_with_custom_style {
	...
	while(<F>){
		...
		if($r=~/^#\s+(Summary|Table of contents|Abstract|Glossary .*|References.*|Documentation .*)/){
			#
			$r=qq|[$1]{custom-style="EFSA_Heading 1 (no number)"}\n|;
		}
		elsif($r=~/^(#+)\s+(\S.*)/){
			#
			$num=length($1);
			$r=qq|[$2]{custom-style="EFSA_Heading $num"}\n|;
		}
		elsif($r=~/^(\s*)([-*])\s+(\S+.*)/){
			#
			$num=($1 eq '')?'1':'2';
			$r=qq|[$3]{custom-style="EFSA_Bullet $num"}\n|;
		}else{
			#
